...
export const GameBoard: React.FC<GameBoardProps> = ({ codeLength, onSubmitGuess, onInvalidGuess, isGameWon, allowRepeats }) => {
  ...
  return (
    <div className={`flex flex-col items-center gap-6 lg:gap-10 w-full ${shake ? 'animate-[shake_0.5s_cubic-bezier(.36,.07,.19,.97)_both]' : ''}`}>
        <style>{`
            @keyframes shake {
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-3 [perspective:1000px]" onPaste={handlePaste}>
          {guess.map((digit, index) => (
            <input
              ...
              className={`w-12 h-14 sm:w-16 sm:h-20 text-3xl sm:text-4xl font-mono text-center font-bold rounded-xl border-2 shadow-sm transition-all duration-200 transform outline-none
                ${
                  digit 
